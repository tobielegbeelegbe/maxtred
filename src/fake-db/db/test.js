...
Mock.onGet("/api/project/all").reply(() => {
  return [200, projects];
});

Mock.onGet("/api/user").reply((config) => {
  const id = config.data;
  const response = projects.find((user) => user.id === id);
  return [200, response];
});

Mock.onPost("/api/user/delete").reply((config) => {
  let user = JSON.parse(config.data);
  const updatedUsers = projects.filter((item) => item.id !== user.id);
...
Mock.onPost("/api/user/update").reply((config) => {
  ...
  if (user.id) {
    const updatedUsers = projects.map((item) => (item.id === user.id ? { ...item, ...user } : item));
    return [200, updatedUsers];
  }
